---
name: lens-mcp-code
description: Install the lens MCP server and register it with Claude Code
user-invocable: true
---

Install the lens MCP server locally and register it with Claude Code so you can call `check_domain`, `check_domains`, and `lens_meta` tools directly from any conversation.
...
```sh
claude mcp add lens -- node ~/.claude/mcp-servers/lens/server.mjs
```
...
> The lens MCP server is installed and registered. Restart Claude Code (or open a new session) to activate it.
>
