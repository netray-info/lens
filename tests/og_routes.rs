...
// ---------------------------------------------------------------------------
// TS-10: Backend failure (error grade) → unknown card with max-age=300
// ---------------------------------------------------------------------------

#[tokio::test]
async fn error_grade_returns_unknown_card_with_short_cache() {
    let counter = Arc::new(AtomicUsize::new(0));
    let mut state = make_state(false, true);
    state.badge_check_fn = Some(mock_check_fn(counter.clone(), "error"));
    ...
    assert!(
        cc.contains("max-age=300"),
        "error grade must have max-age=300, got: {cc}"
    );
    assert!(
        resp.headers().get("etag").is_some(),
        "error grade response must have ETag"
    );
    let bytes = to_bytes(resp.into_body(), 1024 * 1024).await.unwrap();
    assert!(is_png(&bytes), "error grade response must be a valid PNG");
    assert_eq!(
        counter.load(Ordering::SeqCst),
        1,
        "run_check must be called once"
